...
def generate_html_and_json(generate_roots: bool = True):
    ...
    for row in range(df_length):
        ...
        #     html_string += f"""<a class="button_sbs" href="javascript:void(0);" onclick="button_click(this)" data-target="root_family_sbs_{w.pali_}">root family</a>"""

        # if (w.family2 != "" and w.meaning != "" and w.metadata == "") or (w.pali_clean in cf_master_list and w.meaning != ""):
        #     html_string += f"""<a class="button_sbs" href="javascript:void(0);" onclick="button_click(this)" data-target="compound_family_sbs_{w.pali_}">compound family</a>"""
        ...
        html_string += f"""</table>"""
        html_string += f"""<p><a class="link" href="https://onlyoffice.fritz.box/Products/Files/DocEditor.aspx?fileid=99&doc=RFVrcjQ5ODJqamxmemIwbHE2cGF6NVJFK0p0QWFvS2tLcUJDbGVibWdpdz0_Ijk5Ig2">Report a mistake.</a></p>"""
        html_string += f"""</div>"""
        ...
        if w.pos not in indeclinables and w.metadata == "":
            ...
            if w.pos != "sandhi" and w.pos != "idiom":
                ...
                if w.pos in declensions:
                    ...
                    html_string += f"""<a class="link" href="https://onlyoffice.fritz.box/Products/Files/DocEditor.aspx?fileid=99&doc=RFVrcjQ5ODJqamxmemIwbHE2cGF6NVJFK0p0QWFvS2tLcUJDbGVibWdpdz0_Ijk5Ig2">Report a mistake.</a></p>"""
        ...
        #                 html_string += f"""<table class = "table1_sbs">{data_read}</table>"""
...
def generate_roots_html_and_json(data: DataFrames, rsc: ResourcePaths, html_data_list):
    ...
    #         html_string += f"""<div class="root_content_sbs"><p>root. <b>{root}</b><sup>{root_has_verb}</sup>{root_group} {root_sign} ({root_meaning})</p></div>"""
